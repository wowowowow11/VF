...
class ZCAN(object):
    def __init__(self):
        ...
        if platform.system() == "Windows":
            # 1. 获取 zlgcan.py 当前所在的绝对/相对目录路径
            current_dir = os.path.dirname(os.path.abspath(__file__))

            # 2. 拼接出 zlgcan.dll 的完整路径
            dll_path = os.path.join(current_dir, "zlgcan.dll")

            # 3. 针对 Python 3.8+ 增加依赖 DLL 的搜索目录 (非常重要)
            ...
            try:
                ...
                print(f"加载 DLL 失败，请检查 {dll_path} 及同目录下的依赖 DLL 是否存在。")
                raise e
        else:
            # Linux 适配代码
            import ctypes
            try:
                self.__dll = ctypes.cdll.LoadLibrary("libusbcanfd.so")
            except OSError:
                current_dir = os.path.dirname(os.path.abspath(__file__))
                self.__dll = ctypes.cdll.LoadLibrary(os.path.join(current_dir, "libusbcanfd.so"))
